...
<template>
    <section class="flex flex-col gap-8 mt-2">
      <h2 class="text-gray-800 text-2xl md:text-3xl font-bold text-center">PENA KITA<br> <span class="text-xl md:text-2xl text-gray-600">SUDUT PENKASTRAT</span> </h2>
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mx-auto px-8 md:px-12" id="articles">
        <div v-for="(article, index) in articles" :key="index"
          class="bg-white border border-zinc-200 text-zinc-800 rounded-md shadow-lg hover:shadow-2xl transition-all duration-300 grid grid-col-1 gap-2">
...
<script>
    export default {
      ...
      methods: {
        fetchArticles() {
          fetch("https://server.salmanfrds.com/api/articles/pena-kastrat") // Use your backend endpoint
