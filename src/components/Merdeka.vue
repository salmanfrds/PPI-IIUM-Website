...
<template>
    <section aria-label="Kabinet Rangkasa PPI-IIUM" :style="`background-image: url(${activeBackground})`"
        class="min-h-[70vh] bg-cover bg-center xl:min-h-screen flex flex-col justify-center items-center text-slate-800 overflow-hidden pt-6 relative">
        <div class="absolute inset-0 bg-gradient-to-b from-red-50/80 to-white/80 backdrop-brightness-105 z-0"></div>

        <div
            class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-red-500/30 to-transparent rounded-full blur-3xl -z-10 transform translate-x-1/2 -translate-y-1/2">
        </div>
        <div
            class="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tr from-amber-400/20 to-transparent rounded-full blur-3xl -z-10 transform -translate-x-1/3 translate-y-1/3">
        </div>

        <div
            class="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 px-4 md:px-8 lg:px-12 py-12 md:py-16">
            <div class="flex flex-col gap-3 justify-center" data-aos="fade-right" data-aos-duration="1000">
                <div class="flex items-center gap-2 mb-2">
                    <div class="flex h-1.5 shadow-sm">
                        <div class="bg-red-600 w-8 rounded-l-full"></div>
                        <div class="bg-slate-200 w-8 rounded-r-full"></div>
                    </div>
                    <span class="text-sm font-medium uppercase tracking-wider text-red-700">Periode 2024/2025</span>
                </div>
                ...
                <div class="space-y-2 text-center lg:text-left">
                    ...
                    <h2 class="text-2xl md:text-3xl lg:text-4xl text-slate-700 font-medium">Kabinet Rangkasa</h2>
                </div>

                <p class="text-slate-800 leading-relaxed max-w-xl mx-auto lg:mx-0 text-center lg:text-left">
                    Kabinet Rangkasa mencerminkan pondasi yang kuat dan tujuan bersama dalam mengembangkan potensi
                    pelajar Indonesia di IIUM melalui pendidikan, kolaborasi, dan komunikasi yang efektif. Nama Rangkasa
                    sendiri melambangkan struktur yang kokoh dan kesatuan visi dalam mendukung setiap langkah menuju
                    kemajuan bersama, memastikan bahwa setiap individu dapat berkembang secara optimal dalam lingkungan
                    yang mendorong sinergi dan inovasi.
                </p>

                <div class="flex flex-wrap gap-4 justify-center lg:justify-start mt-2">
                    <router-link to="/Kontak"
                        class="text-red-700 bg-white border-2 border-red-700 py-3 px-6 rounded-full shadow-md hover:shadow-lg inline-flex items-center gap-2 font-bold transition-all duration-300 hover:-translate-y-1 hover:bg-red-50">
                        Kontak
                        <i class="fa-solid fa-arrow-right-long text-sm"></i>
                    </router-link>
                    <router-link to="/tentang-kami"
                        class="text-white bg-red-700 border-2 border-red-700 py-3 px-6 rounded-full shadow-md hover:shadow-xl inline-flex items-center gap-2 font-bold transition-all duration-300 hover:-translate-y-1 hover:bg-red-800">
                        Tentang Kami
                        <i class="fa-solid fa-users text-sm"></i>
                    </router-link>
                </div>

                <div class="flex gap-8 justify-center lg:justify-start mt-2 pt-4 border-t-2 border-red-600/50">
                    <div class="text-center">
                        <p class="text-3xl font-bold">300+</p>
                        <p class="text-sm text-red-700">Anggota</p>
                    </div>
                    <div class="text-center">
                        <p class="text-3xl font-bold">20+</p>
                        <p class="text-sm text-red-700">Program</p>
                    </div>
                    <div class="text-center">
                        <p class="text-3xl font-bold">10+</p>
                        <p class="text-sm text-red-700">Kolaborasi</p>
                    </div>
                </div>
            </div>
